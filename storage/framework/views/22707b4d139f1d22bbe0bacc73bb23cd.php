<?php $__env->startSection('title', 'الفيديوهات'); ?>
    ...
    <div class="row align-items-center g-5">
      ...
      <div class="col-lg-6">
        ...
        <div class="vp-ctas d-flex flex-wrap gap-3 mb-5">
          <a href="<?php echo e(route('videos')); ?>" class="btn-vp-pri"><i class="bi bi-play-circle-fill"></i> تصفح الفيديوهات</a>
          <?php if(auth()->guard()->guest()): ?>
          <a href="<?php echo e(route('register')); ?>" class="btn-vp-ghost"><i class="bi bi-person-plus-fill"></i> انضم مجاناً</a>
          <?php endif; ?>
        </div>
        ...
        <div class="vp-stats d-flex flex-wrap gap-3">
          ...
          <?php $__currentLoopData = $vstats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vs): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="vp-stat-card">
            <div style="font-size:1rem;margin-bottom:.3rem"><i class="bi <?php echo e($vs['i']); ?>" style="color:var(--gold)"></i></div>
            <div class="vp-stat-num"><?php echo e($vs['n']); ?></div>
            <div class="vp-stat-lbl"><?php echo e($vs['l']); ?></div>
          </div>
      ...
      <div class="col-lg-6 d-none d-lg-block vp-right">
        <div class="position-relative" style="padding:2.5rem 1.5rem">
          <div class="vp-chip" style="top:-10px;right:-10px;animation:chipFlt 4.5s ease-in-out infinite">
            ...
            <div><div style="font-size:.9rem;font-weight:900;color:#fff;line-height:1"><?php echo e(\App\Models\Video::count()); ?></div><div style="font-size:.63rem;color:rgba(255,255,255,.45)">فيديو متاح</div></div>
          </div>
  ...
  <aside class="vp-sidebar">
    <div class="filter-panel">
      <div class="filter-head">
        ...
        <?php if(request()->hasAny(['search','category','duration'])): ?>
        <a href="<?php echo e(route('videos')); ?>" class="filter-clear-all"><i class="bi bi-x-circle"></i> مسح</a>
        <?php endif; ?>
      </div>
      <div class="filter-body">
        <form method="GET" action="<?php echo e(route('videos')); ?>" id="filterForm">
          ...
          <div class="f-section">
            ...
            <?php $fCats=['psychology'=>'علم النفس','addiction'=>'الإدمان والعلاج','relationships'=>'العلاقات'] ?>
            ...
            <label class="f-check">
              ...
              <span class="f-check-label"><span class="f-check-box"></span><?php echo e($fl); ?></span>
              <span class="f-cnt"><?php echo e(\App\Models\Video::where('category',$fv)->count()); ?></span>
            </label>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          </div>
          ...
          <div class="f-section" style="margin-bottom:1.4rem">
            ...
            <?php $durs=['short'=>'قصير (أقل من 5 دقائق)','medium'=>'متوسط (5-20 دقيقة)','long'=>'طويل (أكثر من 20 دقيقة)'] ?>
            <?php $__currentLoopData = $durs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dv=>$dl): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <label class="f-check">
              <input type="checkbox" name="duration[]" value="<?php echo e($dv); ?>" <?php echo e(in_array($dv,(array)request('duration',[]))?'checked':''); ?>>
              <span class="f-check-label"><span class="f-check-box"></span><?php echo e($dl); ?></span>
            </label>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          </div>
          ...
          <?php if(request()->hasAny(['search','category','duration'])): ?>
          <a href="<?php echo e(route('videos')); ?>" class="f-reset"><i class="bi bi-arrow-clockwise"></i> مسح الكل</a>
          <?php endif; ?>
        </form>
      </div>
    </div>
  ...
  <main class="vp-main">
    ...
    <div class="results-bar" data-sr="up">
      <div class="results-info">
        ...
        <strong><?php echo e($videos->total()); ?></strong> فيديو
        <?php if(request()->hasAny(['search','category','duration'])): ?>
    ...
    <?php if(isset($featuredVideo) && $featuredVideo && !request()->hasAny(['search','category','duration'])): ?>
    <div class="feat-vid" data-sr="up">
      <div class="row g-0">
        <div class="col-md-5">
          <div class="feat-img" style="min-height:260px">
            <?php if($featuredVideo->thumbnail_url): ?>
              <img src="<?php echo e($featuredVideo->thumbnail_url); ?>" alt="<?php echo e($featuredVideo->title); ?>">
            <?php else: ?>
            ...
            <?php if($featuredVideo->category_label ?? null): ?><span class="feat-cat"><?php echo e($featuredVideo->category_label); ?></span><?php endif; ?>
            <span class="feat-dur"><i class="bi bi-clock" style="font-size:.6rem"></i> <?php echo e($featuredVideo->formatted_duration ?? '00:00'); ?></span>
          </div>
        </div>
        <div class="col-md-7">
          <div class="feat-body">
            <div class="feat-title"><?php echo e($featuredVideo->title); ?></div>
            <?php if($featuredVideo->description): ?><p class="feat-desc"><?php echo e(Str::limit($featuredVideo->description,200)); ?></p><?php endif; ?>
            <div class="feat-meta">
              <span class="feat-meta-item"><i class="bi bi-person-fill"></i> <?php echo e($featuredVideo->presenter?->name ?? 'مقدم'); ?></span>
              <span class="feat-meta-item"><i class="bi bi-eye"></i> <?php echo e(number_format($featuredVideo->views??0)); ?></span>
              <?php if($featuredVideo->published_at ?? null): ?><span class="feat-meta-item"><i class="bi bi-calendar3"></i> <?php echo e($featuredVideo->published_at->format('d M Y')); ?></span><?php endif; ?>
            </div>
            <a href="<?php echo e(route('videos.show',$featuredVideo)); ?>"
    ...
    <div class="row g-3">
      <?php $__empty_1 = true; $__currentLoopData = $videos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vi => $video): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      <div class="col-sm-6 col-xl-4" data-sr="up" data-sr-d="<?php echo e(($vi%3)+1); ?>">
        <div class="vc">
          <div class="vc-img">
            <?php if($video->thumbnail_url): ?>
              <img src="<?php echo e($video->thumbnail_url); ?>" alt="<?php echo e($video->title); ?>">
            <?php else: ?>
              <?php $vBgs=['linear-gradient(145deg,#2D5E3A,#3A7D6E)','linear-gradient(145deg,#1a2e20,#2D5E3A)','linear-gradient(145deg,#3A7D6E,#2a6b5d)','linear-gradient(145deg,#C4A882,#a8885e)']; ?>
              <div class="vc-ph" style="background:<?php echo e($vBgs[$vi%4]); ?>"><i class="bi bi-play-circle-fill"></i></div>
            <?php endif; ?>
            <div class="vc-play-overlay">
              <div class="vc-play-btn"><i class="bi bi-play-fill"></i></div>
            </div>
            <?php if($video->category_label ?? null): ?><span class="vc-badge vc-cat-b"><?php echo e($video->category_label); ?></span><?php endif; ?>
            <span class="vc-badge vc-dur-b" style="position:absolute"><i class="bi bi-clock" style="font-size:.6rem"></i> <?php echo e($video->formatted_duration ?? '00:00'); ?></span>
          </div>
          <div class="vc-body">
            <div class="vc-title"><?php echo e($video->title); ?></div>
            <?php if($video->description): ?><div class="vc-desc"><?php echo e($video->description); ?></div><?php endif; ?>
            <div class="vc-auth"><i class="bi bi-person-fill"></i> <?php echo e($video->presenter?->name ?? 'مقدم'); ?></div>
            <div class="vc-foot">
              <div class="vc-stats">
                <span class="vc-stat"><i class="bi bi-eye"></i> <?php echo e(number_format($video->views??0)); ?></span>
                <?php if($video->published_at ?? null): ?><span class="vc-stat"><i class="bi bi-calendar3"></i> <?php echo e($video->published_at->format('M Y')); ?></span><?php endif; ?>
              </div>
              <a href="<?php echo e(route('videos.show',$video)); ?>" class="vc-watch"><i class="bi bi-play-fill" style="font-size:.68rem"></i> شاهد</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-12" data-sr="zoom">
        <div class="vp-empty">
          ...
          <a href="<?php echo e(route('videos')); ?>" class="btn-vp-pri" style="font-size:.84rem;padding:.6rem 1.5rem"><i class="bi bi-arrow-clockwise"></i> عرض الكل</a>
        </div>
      </div>
      <?php endif; ?>
    </div>
    ...
    <div class="vp-pages" data-sr="up"><?php echo e($videos->withQueryString()->links()); ?></div>
    <?php endif; ?>
...
<section style="background:#fff;padding:5rem 0">
  <div class="container">
    ...
    <div class="row g-4">
      ...
      <?php $__currentLoopData = $qcats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $qi=>$qc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <div class="col-md-4" data-sr="up" data-sr-d="<?php echo e($qi+1); ?>">
        <a href="<?php echo e(route('videos',['category'=>$qc['slug']])); ?>" class="qcat">
          <div class="qcat-ico" style="background:<?php echo e($qc['bg']); ?>"><i class="bi <?php echo e($qc['i']); ?>"></i></div>
          <div class="qcat-n"><?php echo e($qc['nm']); ?></div>
          <div class="qcat-d"><?php echo e($qc['d']); ?></div>
          <div class="qcat-cnt-wrap">
            <i class="bi bi-play-circle" style="font-size:.63rem;color:var(--g1)"></i>
            <span class="qcat-cnt"><?php echo e(\App\Models\Video::where('category',$qc['slug'])->count()); ?> فيديو</span>
          </div>
        </a>
      </div>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\saieed\وعي\resources\views\videos\index.blade.php ENDPATH**/ ?>