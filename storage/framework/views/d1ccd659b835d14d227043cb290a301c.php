<?php $__env->startSection('title', 'المقالات الصحية'); ?>
    ...
    <div class="row align-items-center g-5">
      ...
      <div class="col-lg-6">
        ...
        <div class="ap-ctas d-flex flex-wrap gap-3 mb-5">
          <a href="<?php echo e(route('articles')); ?>" class="btn-ap-pri"><i class="bi bi-heart-pulse"></i> تصفح المقالات</a>
          <?php if(auth()->guard()->guest()): ?>
          <a href="<?php echo e(route('register')); ?>" class="btn-ap-ghost"><i class="bi bi-person-plus-fill"></i> انضم مجاناً</a>
          <?php endif; ?>
        </div>
        ...
        <div class="ap-stats d-flex flex-wrap gap-3">
          ...
          <?php $__currentLoopData = $heroStats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $hs): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="ap-stat-card" style="flex:1;min-width:110px">
            <div style="font-size:1rem;margin-bottom:.3rem"><i class="bi <?php echo e($hs['i']); ?>" style="color:var(--gold)"></i></div>
            <div class="ap-stat-num"><?php echo e(number_format($hs['n'])); ?></div>
            <div class="ap-stat-lbl"><?php echo e($hs['l']); ?></div>
          </div>
      ...
      <div class="col-lg-6 d-none d-lg-block ap-right">
        <div class="position-relative" style="padding:2.5rem 1.5rem">
          <div class="ap-chip" style="top:-10px;right:-10px;animation:chipFlt 4.5s ease-in-out infinite">
            ...
            <div><div style="font-size:.9rem;font-weight:900;color:#fff;line-height:1"><?php echo e(\App\Models\Article::published()->count()); ?></div><div style="font-size:.63rem;color:rgba(255,255,255,.45)">مقال موثّق</div></div>
          </div>
  ...
  <aside class="ap-sidebar">
    <div class="filter-panel">
      <div class="filter-head">
        ...
        <?php if(request()->hasAny(['search','category','featured','size'])): ?>
        <a href="<?php echo e(route('articles')); ?>" class="filter-clear-all">
          <i class="bi bi-x-circle"></i> مسح
        </a>
        <?php endif; ?>
      </div>
      <div class="filter-body">
        <form action="<?php echo e(route('articles')); ?>" method="GET" id="filterForm">
          ...
          <div class="f-section">
            ...
            <select name="category" class="f-select">
              ...
              <option value="<?php echo e($cat); ?>" <?php echo e(request('category')==$cat?'selected':''); ?>>
                <?php echo e($cat=='psychology'?'علم النفس':($cat=='addiction'?'الإدمان والعلاج':($cat=='relationships'?'العلاقات':$cat))); ?>
          ...
          <div class="f-section">
            ...
            <select name="featured" class="f-select">
              ...
              <option value="1" <?php echo e(request('featured')=='1'?'selected':''); ?>>مميز فقط</option>
              <option value="0" <?php echo e(request('featured')=='0'?'selected':''); ?>>عادي فقط</option>
            </select>
          </div>
          ...
          <div class="f-section" style="margin-bottom:1.4rem">
            ...
            <select name="size" class="f-select">
              ...
              <option value="small"  <?php echo e(request('size')=='small' ?'selected':''); ?>>قصير (أقل من 5 دقائق)</option>
              <option value="medium" <?php echo e(request('size')=='medium'?'selected':''); ?>>متوسط (5-10 دقائق)</option>
              <option value="large"  <?php echo e(request('size')=='large' ?'selected':''); ?>>طويل (10+ دقائق)</option>
            </select>
          </div>

          <button type="submit" class="f-apply"><i class="bi bi-funnel-fill"></i> تطبيق الفلاتر</button>
          <?php if(request()->hasAny(['search','category','featured','size'])): ?>
          <a href="<?php echo e(route('articles')); ?>" class="f-reset"><i class="bi bi-arrow-clockwise"></i> مسح الكل</a>
          <?php endif; ?>
        </form>
      </div>
    </div>
  ...
  <main class="ap-main">
    ...
    <div class="results-bar" data-sr="up">
      <div class="results-info">
        ...
        <strong><?php echo e($articles->total()); ?></strong> مقال
        <?php if(request()->hasAny(['search','category','featured','size'])): ?>
    ...
    <div class="row g-4">
      <?php $__empty_1 = true; $__currentLoopData = $articles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $article): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      <div class="col-sm-6 col-xl-4" data-sr="up" data-sr-d="<?php echo e(($i%3)+1); ?>">
        <div class="ac">
          ...
          <div class="ac-img">
            ...
            <?php else: ?>
              <?php $iBgs=['linear-gradient(145deg,#2D5E3A,#3A7D6E)','linear-gradient(145deg,#1a2e20,#2D5E3A)','linear-gradient(145deg,#3A7D6E,#2a6b5d)','linear-gradient(145deg,#C4A882,#a8885e)']; ?>
              <div class="ac-img-ph" style="background:<?php echo e($iBgs[$i%4]); ?>"><i class="bi bi-file-richtext-fill"></i></div>
            <?php endif; ?>
            <div class="ac-img-grad"></div>
            <span class="ac-badge ac-cat-b"><?php echo e($article->category_label ?? 'عام'); ?></span>
            ...
            <span class="ac-badge ac-time-b" style="position:absolute">
              <i class="bi bi-clock" style="font-size:.58rem"></i> <?php echo e($article->reading_time ?? 5); ?> دقائق
            </span>
          </div>
          
          <div class="ac-body">
            <div class="ac-author">
              <div class="ac-author-av"><?php echo e(mb_substr($article->author?->first_name ?? 'و', 0, 1)); ?></div>
              <div>
                <div class="ac-author-nm"><?php echo e($article->author?->full_name ?? 'فريق وعي'); ?></div>
                <div class="ac-author-rl"><?php echo e($article->author?->role === 'doctor' ? 'طبيب' : 'متخصص'); ?></div>
              </div>
            </div>
            <h3 class="ac-title">
            ...
            <p class="ac-excerpt">
              <?php echo e($article->excerpt ?? Str::limit(strip_tags($article->body ?? ''), 110)); ?>

            </p>
            <div class="ac-meta">
              <span class="ac-meta-item"><i class="bi bi-eye"></i> <?php echo e(number_format($article->views ?? 0)); ?></span>
              <span class="ac-meta-dot"></span>
              <span class="ac-meta-item"><i class="bi bi-calendar3"></i> <?php echo e($article->published_at?->format('d M Y') ?? now()->format('d M Y')); ?></span>
              <?php if($article->comments_count ?? 0): ?>
              <span class="ac-meta-dot"></span>
              <span class="ac-meta-item"><i class="bi bi-chat"></i> <?php echo e($article->comments_count); ?></span>
              <?php endif; ?>
            </div>
            <div class="ac-foot">
              <a href="<?php echo e(route('articles.show', $article->slug ?? $article->id)); ?>" class="ac-read">
      ...
      <div class="col-12" data-sr="zoom">
        <div class="ap-empty">
          ...
          <a href="<?php echo e(route('articles')); ?>" class="btn-ap-pri" style="font-size:.85rem;padding:.7rem 1.5rem">
  ...
  <div class="container">
    ...
    <div class="row g-4">
      ...
      <div class="col-md-4" data-sr="up" data-sr-d="<?php echo e($qi+1); ?>">
        <a href="<?php echo e(route('articles',['category'=>$qc['slug']])); ?>" class="qcat">
          ...
          <div class="qcat-cnt-wrap">
            ...
            <span class="qcat-cnt"><?php echo e(\App\Models\Article::where('category',$qc['slug'])->published()->count()); ?> مقال</span>
          </div>
        </a>
      </div>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\saieed\وعي\resources\views\articles\index.blade.php ENDPATH**/ ?>