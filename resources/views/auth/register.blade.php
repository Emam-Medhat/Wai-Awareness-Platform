...
@section('auth-title', 'مرحباً بك في منصة وعي!')
@section('auth-subtitle', 'انضم إلى مجتمعنا وابدأ رحلتك نحو الوعي والتطوير')
...
<div class="w-full">
    ...
    <form action="{{ route('register.post') }}" method="POST" class="space-y-8" data-aos="fade-up" data-aos-duration="1000">
        @csrf
                ...
                <div class="bg-white bg-opacity-90 backdrop-filter backdrop-blur-lg rounded-3xl shadow-2xl p-8 border border-white border-opacity-20 h-full transform transition-all duration-300 hover:scale-[1.02] hover:shadow-3xl">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            >{{ old('bio') }}</textarea>
                            <p class="text-xs text-gray-500 mt-1">{{ 250 - strlen(old('bio') ?? 0) }} حرف متبقي</p>
                            @error('bio')
                                <p class="mt-2 text-sm text-red-600 flex items-center">
                                    <i class="fas fa-exclamation-circle ml-2"></i>
                                    {{ $message }}
...
@section('auth-footer')
    <div class="text-center mt-8" data-aos="fade-up" data-aos-duration="1400">
        <span class="text-gray-600 text-lg">
            لديك حساب بالفعل؟
            <a href="{{ route('login') }}" class="font-semibold text-primary hover:text-primary-700 transition">
                تسجيل الدخول
            </a>
        </span>
    </div>
