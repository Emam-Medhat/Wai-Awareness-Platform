...
<div class="as-bc">
  <div class="container">
    <ol class="bc-ol">
      <li><a href="{{ route('home') }}" class="bc-a"><i class="bi bi-house-fill"></i> الرئيسية</a></li>
      <li><i class="bi bi-chevron-left bc-sep" style="font-size:.6rem"></i></li>
      <li><a href="{{ route('articles') }}" class="bc-a"><i class="bi bi-newspaper"></i> المقالات</a></li>
      <li><i class="bi bi-chevron-left bc-sep" style="font-size:.6rem"></i></li>
      <li><span class="bc-cur">{{ $article->title }}</span></li>
    </ol>
  </div>
</div>
...
<div class="as-hero">
  @if($article->image_url ?? null)
    <img src="{{ $article->image_url }}" alt="{{ $article->title }}" class="as-hero-img">
  @else
    @php $pBgs=['linear-gradient(145deg,#2D5E3A,#3A7D6E)','linear-gradient(145deg,#1a2e20,#2D5E3A)','linear-gradient(145deg,#3A7D6E,#2a6b5d)']; @endphp
    <div class="as-hero-ph" style="background:{{ $pBgs[abs(crc32($article->slug??'x'))%3] }}">
  ...
  <div class="h-badges">
    @if($article->category_label ?? null)
      <span class="h-badge h-cat">{{ $article->category_label }}</span>
    @endif
  ...
  <div class="as-hero-body">
    <h1 class="as-hero-title">{{ $article->title }}</h1>
    <div class="as-hero-meta">
      <span class="h-meta-chip">
        <div class="h-author-av">
          @if($article->author_avatar_url ?? null)
            <img src="{{ $article->author_avatar_url }}" alt="">
          @else
            {{ mb_substr($article->author_name ?? $article->author?->first_name ?? 'و', 0, 1) }}
          @endif
        </div>
        <span style="color:#fff;font-weight:700">{{ $article->author_name ?? $article->author?->full_name ?? 'فريق وعي' }}</span>
        <span style="color:rgba(255,255,255,.4);font-size:.65rem">·</span>
        <span style="color:rgba(255,255,255,.6);font-size:.7rem">{{ $article->author_title ?? ($article->author?->role === 'doctor' ? 'طبيب' : 'متخصص') }}</span>
      </span>
      <span class="h-meta-chip"><i class="bi bi-calendar3"></i> {{ ($article->published_at ?? now())->format('d M Y') }}</span>
      <span class="h-meta-chip"><i class="bi bi-clock"></i> {{ $article->reading_time ?? 5 }} دقائق</span>
      <span class="h-meta-chip"><i class="bi bi-eye"></i> {{ number_format($article->views ?? 0) }}</span>
    </div>
  </div>
</div>
        ...
        <div class="as-actions" data-sr="up">
          <div class="act-btns">
            <button class="act-btn" id="heartBtn" onclick="toggleHeart(this)">
              ...
              <span id="heartCnt">{{ $article->likes_count ?? 0 }}</span>
            </button>
          ...
          <div class="as-tags">
            ...
            <a href="{{ route('articles',['tag'=>$tag]) }}" class="as-tag">
              <i class="bi bi-hash" style="font-size:.58rem"></i>{{ $tag }}
            </a>
            @endforeach
          </div>
          @endif
          ...
          <div class="author-bio">
            ...
            <div style="flex:1">
              <div style="display:flex;align-items:center;flex-wrap:wrap;gap:.5rem;margin-bottom:.25rem">
                <div class="bio-name">{{ $article->author_name ?? $article->author?->full_name ?? 'فريق وعي' }}</div>
                <span class="bio-role"><i class="bi bi-patch-check-fill"></i>{{ ($article->author?->role ?? '') === 'doctor' ? 'طبيب معتمد' : 'متخصص معتمد' }}</span>
              </div>
              <div class="bio-sub">{{ $article->author_title ?? $article->author?->title ?? 'متخصص في الصحة النفسية والتوعية' }}</div>
              <p class="bio-text">{{ $article->author_bio ?? $article->author?->bio ?? 'كاتب متخصص في مجال الصحة النفسية والتوعية المجتمعية، يهدف إلى نشر الوعي وتقديم المحتوى العلمي الموثوق.' }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="as-comments" data-sr="up" data-sr-d="2">
          <div class="comments-hd">
            ...
            <div>
              ...
              <div class="comments-hd-cnt">{{ $article->comments_count ?? 0 }} تعليق</div>
            </div>
          </div>

          {{-- Form --}}
          @auth
          <form action="{{ route('articles.comment.store', $article->slug) }}" method="POST" style="margin-bottom:1.6rem">
            @csrf
          ...
          <div class="cm-login" style="margin-bottom:1.5rem">
            ...
            <a href="{{ route('login') }}" style="display:inline-flex;align-items:center;gap:.42rem;padding:.62rem 1.5rem;background:linear-gradient(135deg,var(--g1),var(--teal));color:#fff;font-size:.82rem;font-weight:700;border-radius:12px;text-decoration:none;box-shadow:0 5px 18px rgba(45,94,58,.25)">
          ...
          <div>
            @if($article->comments_count ?? 0 > 0)
              ...
              <div class="cm-item">
                <div class="cm-av">
                  @if($cm->user?->avatar ?? null)<img src="{{ $cm->user->avatar }}" alt="">
                  @else{{ mb_substr($cm->user?->name ?? 'م',0,1) }}@endif
                </div>
                <div style="flex:1">
                  <div class="cm-bubble">
                    <div class="cm-head">
                      <span class="cm-name">{{ $cm->user?->full_name ?? 'مستخدم' }}</span>
                      <span class="cm-time">{{ $cm->created_at?->diffForHumans() }}</span>
                    </div>
                    <p class="cm-text">{{ $cm->body }}</p>
                  </div>
                  <div class="cm-acts">
      ...
      <aside class="as-sidebar">
        ...
        <div class="as-card" data-sr="right" data-sr-d="1">
          ...
          <div class="as-card-body" style="padding-top:.7rem">
            ...
            <div class="rel-item">
              <div class="rel-thumb">
                @if($rel->image_url ?? null)<img src="{{ $rel->image_url }}" alt="{{ $rel->title }}">
                @else<div class="rel-ph" style="background:linear-gradient(135deg,var(--g1),var(--teal))"><i class="bi bi-file-richtext-fill"></i></div>@endif
              </div>
              <div style="flex:1">
                <a href="{{ route('articles.show',$rel->slug) }}" class="rel-title">{{ $rel->title }}</a>
                <div class="rel-meta"><i class="bi bi-clock" style="color:var(--gold);font-size:.6rem"></i> {{ $rel->reading_time ?? 5 }} دقائق</div>
              </div>
            </div>
            @empty
            @php $relColors=['linear-gradient(135deg,#2D5E3A,#3A7D6E)','linear-gradient(135deg,#1a2e20,#2D5E3A)','linear-gradient(135deg,#C4A882,#a8885e)'] @endphp
            @for($ri=0;$ri<3;$ri++)
            <div class="rel-item">
              <div class="rel-thumb"><div class="rel-ph" style="background:{{ $relColors[$ri] }}"><i class="bi bi-file-richtext-fill"></i></div></div>
              <div style="flex:1">
                <span class="rel-title" style="display:block">مقال ذو صلة {{ $ri+1 }}</span>
        ...
        <div class="as-card" data-sr="right" data-sr-d="2">
          ...
          <div class="as-card-body" style="padding-top:.5rem">
            ...
            @foreach($sideCats as $sc)
            <a href="{{ route('articles',['category'=>$sc['slug']]) }}" class="cat-row">
              <div class="c-left">
                <div class="c-ico" style="background:{{ $sc['bg'] }}"><i class="bi {{ $sc['i'] }}"></i></div>
                {{ $sc['n'] }}
              </div>
              <span class="c-cnt">{{ \App\Models\Article::where('category',$sc['slug'])->published()->count() }}</span>
            </a>
            @endforeach
          </div>
        </div>
