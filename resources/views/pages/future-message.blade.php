...
<section class="messages-sec">
  <div class="messages-container">
    ...
    <div class="messages-grid">
      ...
      <div class="message-card" data-reveal data-reveal-delay="{{ $loop->index + 1 }}">
        <div class="message-date">
          <i class="bi bi-calendar-event"></i>
          <span>{{ $message->remind_at->format('Y-m-d H:i') }}</span>
        </div>
        <div class="message-content">{{ $message->content }}</div>
        <div class="message-status {{ $message->is_sent ? 'sent' : 'pending' }}">
          @if($message->is_sent)
        ...
        <div class="message-actions">
          <form action="{{ route('future-message.destroy', $message) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذه الرسالة؟')">
            @csrf
            @method('DELETE')
...
<section class="form-sec">
  ...
  <div class="form-container">
    ...
    <div class="form-card" data-reveal data-reveal-delay="1">
      <form action="{{ route('future-message.store') }}" method="POST">
        @csrf
        <div class="form-group">
          <label class="form-label" for="content">محتوى الرسالة</label>
          <textarea name="content" id="content" class="form-input form-textarea" placeholder="اكتب رسالتك هنا..." required>{{ old('content') }}</textarea>
          @error('content')
        ...
        <div class="form-group">
          ...
          <input type="datetime-local" name="remind_at" id="remind_at" class="form-input" required value="{{ old('remind_at') }}">
          @error('remind_at')
          <div style="color:#ef4444;font-size:.875rem;margin-top:.5rem">
            <i class="bi bi-exclamation-circle me-1"></i>{{ $message }}
          </div>
