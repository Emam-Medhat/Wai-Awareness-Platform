        ...
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($testimonials as $index => $testimonial)
                    <div class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="{{ $index * 100 }}">
                        <!-- Card Header -->
                        ...
                        <div class="p-8">
                            ...
                            <div class="flex items-center justify-center border-t pt-6">
                                <img src="{{ $testimonial['avatar'] }}" alt="{{ $testimonial['name'] }}" class="w-16 h-16 rounded-full border-4 border-primary border-opacity-20 shadow-lg">
                                <div class="mr-4">
                ...
                @foreach($videoTestimonials as $index => $video)
                    <div class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="{{ $index * 100 }}">
                        <div class="relative h-56 overflow-hidden">
                            <img src="{{ $video['thumbnail'] }}" alt="{{ $video['title'] }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                        ...
                        <div class="p-8">
                            <h4 class="text-xl font-bold text-gray-800 mb-3 group-hover:text-primary transition">{{ $video['title'] }}</h4>
                            <p class="text-gray-600 mb-6 leading-relaxed">{{ $video['description'] }}</p>
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <img src="{{ $video['avatar'] }}" alt="{{ $video['client_name'] }}" class="w-12 h-12 rounded-full border-3 border-primary border-opacity-20 shadow-md ml-3">
                                    <div>
                                        <p class="text-gray-800 font-semibold">{{ $video['client_name'] }}</p>
                                        <p class="text-gray-500 text-sm">{{ $video['client_role'] }}</p>
                                    </div>
                                </div>
        ...
        <div class="container mx-auto px-4 text-center relative z-10">
            <div data-aos="fade-up" data-aos-duration="1000">
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="{{ route('register') }}" class="bg-white text-secondary px-8 py-4 rounded-lg font-semibold hover:bg-gray-100 transition inline-flex items-center justify-center transform hover:scale-105 shadow-lg">
                        <i class="fas fa-user-plus ml-2"></i>
                        ابدأ الآن
                    </a>
                    <a href="{{ route('contact') }}" class="bg-transparent border-2 border-white text-white px-8 py-4 rounded-lg font-semibold hover:bg-white hover:text-secondary transition inline-flex items-center justify-center transform hover:scale-105">
